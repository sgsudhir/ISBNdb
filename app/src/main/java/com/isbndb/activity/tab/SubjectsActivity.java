...
public class SubjectsActivity extends Activity implements View.OnClickListener{
    ...
    String subjectsNames[], subjectsIds[];

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.tab_subjects);
        stringJsonObject = getIntent().getStringExtra("json_object");
        buttonPageNext = (Button) findViewById(R.id.button_page_next_tab_subjects);
        buttonPagePrevious = (Button) findViewById(R.id.button_page_previous_tab_subjects);
        textViewPages = (TextView) findViewById(R.id.text_view_pages_tab_subjects);
        listViewSubjects = (ListView) findViewById(R.id.list_view_subjects);
        buttonPagePrevious.setOnClickListener(this);
        buttonPageNext.setOnClickListener(this);
        if (getMetaData(stringJsonObject))
            if (getSubjectsDetails(stringJsonObject))
                listViewSubjects.setAdapter(new ListViewAdapter(1, subjectsNames, null, null, this));
    }
    ...
    protected boolean getMetaData(String jsonObjectString) {
        ...
        try {
            ...
            subjectsNames = new String[currentPageData];
            subjectsIds = new String[currentPageData];
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    protected boolean getSubjectsDetails(String jsonObjectString) {
        ...
        try {
            ...
            for (int i=0; i<array.length(); i++) {
                ...
                subjectsNames[i] = innerObject.getString("name");
                subjectsIds[i] = innerObject.getString("subject_id");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return false;
